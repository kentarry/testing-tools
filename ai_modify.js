// ═══════════════════════════════════════════════════
// AI 自動改財產 — v2 Optimized
// ✅ Skill shortcuts for one-click multi-step
// ✅ Batch mode for multi-line commands
// ✅ Parallel execution for speed
// ✅ Reduced DOM re-renders
// ✅ Zero token consumption (local keyword matching)
// ═══════════════════════════════════════════════════

const AI_TOOL_REGISTRY = [];

function _registerAiTools() {
  if (window.PLATFORM_AIO) window.PLATFORM_AIO.tools.forEach(t => AI_TOOL_REGISTRY.push({ platform: window.PLATFORM_AIO, tool: t, game: '明星3缺1' }));
  if (window.PLATFORM_TMD) {
    window.PLATFORM_TMD.tools.forEach(t => { if (t.id !== 'tmd_reload') AI_TOOL_REGISTRY.push({ platform: window.PLATFORM_TMD, tool: t, game: '滿貫大亨' }); });
  }
  if (window.PLATFORM_VF) window.PLATFORM_VF.tools.forEach(t => AI_TOOL_REGISTRY.push({ platform: window.PLATFORM_VF, tool: t, game: 'Vegas Frenzy' }));
}

// ── Skill definitions (multi-step shortcuts) ──
const AI_SKILLS = {
...
function renderAiModifyTool() {
  ...
  panel.innerHTML = `
    <div class="p-head">
      <div class="p-title">🤖 AI 自動改財產 <span style="font-size:12px;color:var(--t3);margin-left:8px">v2 — Optimized</span></div>
      <div class="p-desc">輸入帳號與需求，AI 自動辨識並執行。支援 <b>技能一鍵執行</b> 和 <b>批量模式</b>。
範例：「明星 ray15 改金幣 500000」「滿貫 ray10 vip 8」「vf 163436 改錢 100000」
CLI：<code>node AI自動改財產/ai_cli.js --help</code></div>
    </div>
    ${skillHtml}
    ...
    <div class="ai-input-area">
      <div class="ai-input-row">
        <textarea class="fi ai-input" id="aiAccountInput" placeholder="帳號（每行一個）&#10;ray1&#10;ray2&#10;ray3" style="width:160px;flex-shrink:0;min-height:60px;resize:vertical;font-family:inherit;line-height:1.5"></textarea>
      ...
      <div style="display:flex;gap:8px;margin-top:6px">
        ...
        <label style="font-size:12px;color:var(--accH);display:flex;align-items:center;gap:4px;margin-left:12px" title="免開 RUN.BAT，適合放 GitHub Pages">
          <input type="checkbox" id="aiCloudProxy"> ☁️ 雲端代理 (免伺服器)
        </label>
        <button class="btn btn-rst" style="font-size:11px;padding:3px 10px;margin-left:auto" onclick="window._aiChatHistory=[];_renderAiHistory()">🗑️ 清除紀錄</button>
      </div>
    </div>
    ${toolHtml}`;
  ...
  if (location.hostname.includes('github.io') || location.protocol === 'file:') {
    const cb = document.getElementById('aiCloudProxy');
...
function _renderAiHistory() {
  ...
  c.innerHTML = window._aiChatHistory.map(_buildMsgHtml).join('');
...
// ── Execute skill (parallel, multi-account) ──
async function aiRunSkill(skillName) {
  ...
  window._aiChatHistory.push({ role: 'user', account: accounts.join(', '), text: `⚡ 技能: ${skill.desc}  (${accounts.length} 帳號)` });
  window._aiChatHistory.push({ role: 'bot', ok: true, game: 'Skill', toolName: skillName, text: `⏳ 執行中... (${accounts.length} 帳號，並行模式)` });
  _renderAiHistory();

  const startTime = Date.now();

  // Execute all accounts in parallel, each account's steps also in parallel
  ...
  window._aiChatHistory[window._aiChatHistory.length - 1] = {
...
async function aiSubmit() {
  ...
  window._aiChatHistory.push({ role: 'user', account: accounts.join(', '), text: `${request}  (${accounts.length} 帳號)` });
  requestEl.value = '';

  // Validate with first account
  const testMatches = _aiParseRequest(accounts[0], request);
  if (testMatches.length === 0) {
    window._aiChatHistory.push({ role:'bot', ok:false, toolName:'無法辨識', text:'❌ 無法辨識需求', detail:'範例：「明星 改金幣 500000」「滿貫 vip 8」「vf 改錢 100000」' });
    _renderAiHistory();
    toast('❌ 無法辨識需求', 'err');
    return;
  }
  ...
  const startTime = Date.now();
  const toolNames = testMatches.map(m => m.tool.name).join(', ');

  window._aiChatHistory.push({ role:'bot', ok:true, game:'AI', toolName:'批次執行',
    text:`⏳ 批次執行 ${accounts.length} 帳號 × ${testMatches.length} 工具 (並行模式)...` });
  _renderAiHistory();

  // Execute all accounts × all tools in parallel
  const allResults = await Promise.all(accounts.map(async account => {
...
// ── Platform definition ──
window.PLATFORM_AI_MODIFY = {
  id: 'ai_modify', name: 'AI自動改財產', icon: '🤖', color: '#10b981',
  base: '', method: 'LOCAL', apiType: 'custom-render',
  tools: [{ id:'ai_auto_modify', cat:'AI自動改財產', icon:'🤖', name:'AI 自動改財產', desc:'輸入帳號與需求，AI自動辨識並執行', customRender: renderAiModifyTool, fields:[] }]
};
